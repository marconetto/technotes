...
// this code is for squared matrix
...
void matrix_multiplication(double *a, double *b, double *c,
                           int matrix_size, int rows){


    for (int k=0; k<matrix_size; k++)
       for (int i=0; i<rows; i++) {
          for (int j=0; j<matrix_size; j++)
             c[i*matrix_size+k] = c[i*matrix_size+k] +
...
void handle_arguments(int argc, char *argv[], int *matrix_size,
                      int *verbose, int *num_interactions, int *numworkers,
                      int numtasks, int taskid) {

    int rc;

    if (argc < 2) {
         if (taskid == MASTER) show_usage(argv[0]);
    ...
    *numworkers = numtasks-1;
    ...
    if (argc > 2) {
        if (is_integer(argv[2])) *num_interactions = atoi(argv[2]);
        else if (!strcmp(argv[2], "v")) *verbose = 1;
    }
    ...
    *matrix_size = atoi(argv[1]);
    if (taskid == MASTER) printf("matrix size= %d\n",*matrix_size);
}
...
int main (int argc, char *argv[])
{
    ...
    if (taskid == MASTER) {
        ...
        for (int inter=0 ; inter < num_interactions ; inter++){
            ...
            for (dest=1; dest<=numworkers; dest++) {
                ...
                MPI_Send(a+offset*matrix_size, rows*matrix_size, MPI_DOUBLE, dest, mtype,
                         MPI_COMM_WORLD);
                MPI_Send(b, matrix_size*matrix_size, MPI_DOUBLE, dest, mtype, MPI_COMM_WORLD);
            ...
            for (i=1; i<=numworkers; i++) {
                ...
                MPI_Recv(c+offset*matrix_size, rows*matrix_size, MPI_DOUBLE, source, mtype,
                        MPI_COMM_WORLD, &status);
            }
            if (verbose) print_matrix(c, matrix_size, matrix_size, "Final Matrix C");
    ...
    if (taskid > MASTER) {
        ...
        for (int inter=0 ; inter < num_interactions ; inter++){
            ...
            MPI_Recv(a, rows*matrix_size, MPI_DOUBLE, MASTER, mtype, MPI_COMM_WORLD, &status);
            MPI_Recv(b, matrix_size*matrix_size, MPI_DOUBLE, MASTER, mtype, MPI_COMM_WORLD, &status);
            ...
            MPI_Send(c, rows*matrix_size, MPI_DOUBLE, MASTER, mtype, MPI_COMM_WORLD);
        }
   }
   MPI_Finalize();
}
